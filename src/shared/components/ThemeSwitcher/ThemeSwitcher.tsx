...
export function ThemeSwitcher({
  ...
  const switchPositionCss = switchPosition
    ? styles[switchPosition.toString() + 'Position']
    : '';
  ...
  function handleSwitch(value: string) {
    const newPosition: ESwitchPosition =
      ESwitchPosition[value as keyof typeof ESwitchPosition];

    if (
      newPosition === ESwitchPosition.center &&
      switchPosition === ESwitchPosition.left
    ) {
      setAnimation(EAnimation['leftToCenter']);
    } else if (
      newPosition === ESwitchPosition.right &&
      switchPosition === ESwitchPosition.center
    ) {
      setAnimation(EAnimation['centerToRight']);
    } else if (
      newPosition === ESwitchPosition.center &&
      switchPosition === ESwitchPosition.right
    ) {
      setAnimation(EAnimation['rightToCenter']);
    } else if (
      newPosition === ESwitchPosition.left &&
      switchPosition === ESwitchPosition.center
    ) {
